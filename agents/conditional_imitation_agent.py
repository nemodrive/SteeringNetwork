...
class ConditionalImitationAgent(LearningAgent):  # ++ Extend Learning agent
    ...
    def _train(self, data_loader):
        ...
        for batch_idx, (images, speed, target_vector,
            ...
            output = to_cuda(torch.zeros((mask.shape[0], 3)), use_cuda)
    ...
    def _get_criterion(self, branch_outputs, speed_outputs, branch_target,
        ...
        loss1 = (branch_target - branch_outputs) * (
            branch_target - branch_outputs)
        loss1 = loss1.sum(dim=0)

        loss_weights = self.cfg.train.loss_weights

        loss1 = loss_weights[0] * loss1[0] + (
            loss_weights[1] * loss1[1] + loss_weights[2] * loss1[2])
    ...
    def _show_activation_image(self, raw_activation, image_activation):
        ...
        cv2.imwrite(self.act_dir + "/activations_img_" + str(self.nr_img) + ".jpg",
    ...
    def run_image(self, image_raw, speed, cmd):
        ...
        return output[0], output[1], output[2], speed_output[
            0] * 90, activation_map
    ...
    def run_1step(self, image_raw, speed, cmd):
        ...
        output = output.data.cpu()[0].numpy()
        speed_output = speed_output.data.cpu()[0].numpy()
        return output[0], output[1], output[2], speed_output[0] * 90
    ...
    def _control_function(self, image_input_raw, real_speed, control_input):
        ...
        steer, acc, brake = output[0], output[1], output[2]
