...
tf.app.flags.DEFINE_string(
    'video_index', '/data/nx-bdd-20160929/video_filtered_index_38_60_sec.txt',
    'filtered video indexing')
tf.app.flags.DEFINE_string('output_directory',
                           '/data/nx-bdd-20160929/tfrecord_fix_speed/',
...
tf.app.flags.DEFINE_integer('truncate_frames', 36 * 15,
                            'Number of frames to leave in the saved tfrecords')
tf.app.flags.DEFINE_string('temp_dir_root', '/tmp/',
                           'the temp dir to hold ffmpeg outputs')

tf.app.flags.DEFINE_boolean('low_res', False,
                            'the data we want to use is low res')
...
def read_one_video(video_path, jobid):
    ...
    if FLAGS.low_res:
        ...
        image_buff = image_buff.reshape(FLAGS.truncate_frames, HEIGHT, WIDTH,
                                        3)
        ...
        for subdir, dirs, files in os.walk(cache_images):
            for f in sorted(files):
                with open(os.path.join(subdir, f), 'rb') as f:
                    image_data = f.read()
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    ...
                    'image/speeds': _float_feature(
                        speeds.ravel().tolist()),  # ravel l*2 into list
                }))
    else:
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    ...
                    'image/channel': _int64_feature(3),
                    'image/class/video_name': _bytes_feature([bytes(video_path, 'utf-8')]),
                    'image/format': _bytes_feature([bytes('JPEG', 'utf-8')]),
                    'image/encoded': _bytes_feature(image_list),
                    'image/speeds': _float_feature(
                        speeds.ravel().tolist()),  # ravel l*2 into list
                }))
...
def parse_path(video_path, jobid):
    ...
    cache_images = os.path.join(FLAGS.temp_dir_root,
                                "prepare_tfrecords_image_temp_" + str(jobid))
